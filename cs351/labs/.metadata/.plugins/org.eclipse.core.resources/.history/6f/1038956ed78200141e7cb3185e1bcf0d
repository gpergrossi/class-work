/*
 * Explicit 2D list structure allocator, designed and implemented by Gregary Pergrossi.
 *
 * Possible improvements :
 * - better (sub-linear) searching for size lists (binary tree?)
 * - special blocks for small allocations to avoid minimum 40 byte block restriction
 * - reorganize and compartmentalize larger functions
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
struct mm_node_s {
	...
	// Unallocated chunks
	mm_node *prev;			// Previous (smaller) size bin (null if first)
	mm_node *next;			// Next (larger) size bin (null if last)
	mm_node *same;			// Pointer to d-list of same size elements (null if no others)

	// Footer size_t
};

mm_node *first_node;		// Smallest bin (null if no bins)
void *wilderness;			// Begining of last free chunk before edge of heap
...
/**
 * Adds a block to the free-structure.
 * The free structure begins with an ordered doubly linked list whose first node
 * is pointed to by the first_node variable. Each item in the list is a unique
 * size of block that has been freed and may be used in the future. If more
 * than one of the same size block is available, they are stacked in a secondary
 * doubly linked list, starting from the ->same of the node in the original d-list (icon).
 * The icon->same points to a doubly linked list terminated on front and end by NULLs.
 * The first element of the secondary list holds its own ->same pointer back to the
 * icon. (This is needed so that it can be removed and update references). The
 * structure needs to support removal from any location due to coalesces potentially
 * consuming blocks of any size.
 */
...
/**
 * Removes a block from the semi-complicated free structure.
 */
